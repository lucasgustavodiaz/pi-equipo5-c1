...
export function FormEdit() {
  ...
  const urlGetYacht = 'http://localhost:8081/api/???'
  async function fetchData() {
    try {
      const response = await fetch(urlGetYacht)
      if (!response.ok) {
        throw new Error(
          'Error al intentar cargar el registro: . Response: ' +
            response.status
        )
      }
  ...
  async function handleSubmit(e) {
    ...
    console.log(JSON.stringify(updatedYacht))
    const urlPostEdit = 'http://localhost:8081/api/???'
    const opcion = confirm(
      `Seguro que desea editar el registro del yate: ${name}`
    )
    if (opcion) {
      try {
        const response = await fetch(urlPostEdit, {
          method: 'POST',
          headers: {
